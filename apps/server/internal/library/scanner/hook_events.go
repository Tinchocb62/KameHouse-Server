...
// ScanMediaFetcherStartedEvent is triggered right before KameHouse starts fetching media to be matched against the local files.
...
type ScanMatchingStartedEvent struct {
	...
	NormalizedMedia []*dto.NormalizedMedia `json:"normalizedMedia"`
...
type ScanLocalFileMatchedEvent struct {
	...
	Match     *dto.NormalizedMedia `json:"match"`
...
type ScanLocalFileHydrationStartedEvent struct {
	hook_resolver.Event
	LocalFile *dto.LocalFile       `json:"localFile"`
	Media     *dto.NormalizedMedia `json:"media"`
}
