...
func (h *Handler) HandleTorrentClientDownload(c echo.Context) error {
	...
	var err error
	completeAnime, err = h.App.Metadata.AnilistPlatformRef.Get().GetAnimeWithRelations(c.Request().Context(), b.Media.ID)
	if err != nil {
	...
	go func() {
		...
		if b.Media != nil {
			...
			err = h.App.Metadata.AnilistPlatformRef.Get().AddMediaToCollection(c.Request().Context(), []int{b.Media.ID})
			if err != nil {
...
func (h *Handler) HandleTorrentClientAddMagnetFromRule(c echo.Context) error {
	...
	rule, err := db.GetAutoDownloaderRule(h.App.Database, b.RuleId)
	if err != nil {
		return h.RespondWithError(c, err)
	}
