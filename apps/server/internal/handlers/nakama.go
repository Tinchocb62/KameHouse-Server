...
func (h *Handler) HandleGetNakamaAnimeLibrary(c echo.Context) error {
	...
	lfs = lo.Filter(lfs, func(lf *dto.LocalFile, _ int) bool {
...
func (h *Handler) getFilteredLocalFiles(mediaId int) ([]*dto.LocalFile, error) {
	lfs, _, err := db.GetLocalFiles(h.App.Database)
	if err != nil {
		return nil, err
	}
	...
	return lo.Filter(lfs, func(lf *dto.LocalFile, _ int) bool {
...
func (h *Handler) buildNakamaLocalFiles(lfs []*dto.LocalFile) (*nakama.NakamaLocalFiles, error) {
	animeCollection, err := h.App.GetAnimeCollection(false)
	if err != nil {
		return nil, err
	}
...
func (h *Handler) HandleNakamaPlayVideo(c echo.Context) error {
	...
	err = h.App.NakamaManager.PlayHostAnimeLibraryFile(b.Path, c.Request().Header.Get("User-Agent"), b.ClientId, models.ToLibraryMedia(media), b.AniDBEpisode, b.ForcePlaybackMethod)
	if err != nil {
		return h.RespondWithError(c, err)
	}
...
func (h *Handler) HandleNakamaProxyStream(c echo.Context) error {
	...
	if streamType == "debrid" {
		...
		req.Header.Set("X-KameHouse-Nakama-Token", h.App.Settings.GetNakama().RemoteServerPassword)
...
//
//	@summary creates a KameHouse Room and connects to it as host.
//	@desc This creates a room on the KameHouse Rooms relay server and connects as the host. Peers can then join using the returned URL.
