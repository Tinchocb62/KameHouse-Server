...
(() => {
  ...
  function createCacheablePlugin(statuses) {
    return new CacheableResponsePlugin({ statuses });
  }
  function createExpirationPlugin(maxEntries, maxAgeSeconds) {
  ...
  var opfsRoot = null;
  var OPFS_CHUNK_DIR_NAME = "media_chunks";
  ...
  registerRoute(
    ({ request, url }) => url.pathname.startsWith("/api/") && !url.pathname.includes("/mediastream/") && !url.pathname.includes("/video-thumbnail") && request.method === "GET",
    new NetworkFirst({
